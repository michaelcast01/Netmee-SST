...
export type InspectionPdfData = {
  ...
  aiReviews: Array<{ modelVersion:string; confidence:number|null; summary:string; decision:string|null; validator:string|null; notes:string|null; validatedAt:Date|null }>;
...
export async function createInspectionPdf(data:InspectionPdfData){
  ...
  heading("Analisis IA y decision humana"); if(!data.aiReviews.length)line("Analisis","No registrados"); for(const review of data.aiReviews){line("Modelo",`${review.modelVersion} - confianza ${review.confidence===null?"sin dato":`${Math.round(review.confidence*100)} %`}`);line("Resultado",review.summary);line("Decision SST",review.decision?`${review.decision}${review.validator?` por ${review.validator}`:""}${review.validatedAt?` - ${review.validatedAt.toLocaleString("es-CO")}`:""}`:"Pendiente de validacion");if(review.notes)line("Nota SST",review.notes);} y-=8;
